...
Até agora, temos aprendido como escrever programas e comunicar nossas interpretações para a {\bf unidade central de processamento} usando execução condicional, funções e iterações.  

Temos aprendido como criar e usar estruturas de dados na {\bf Memoria principal}. O CPU e memória são onde nosso software trabalha e executa. É onde todo o ``thinking'' acontece.

Mas se você se lembrar da nossa discussão sobre arquitetura de hardwares, 
uma vez que a energia está desliga, qualquer coisa armazenada na CPU ou memória principal são apagados. 
Então, até agora, nossos programas tem sido apenas transitórios e divertidos para aprender Python.

\beforefig
\centerline{\includegraphics[height=2.50in]{figs2/arch3.eps}}
\afterfig

Neste capítulo, iniciaremos o trabalho como {\bf memória secundária}
(ou arquivos).
Memória secundária não é apagada mesmo quando a energia é interrompida.
ou no caso de um flash drive USB, os dados que escrevemos nossos programas pode ser removidos de um sistema e transportados para outro sistema.

Vamos primeiramente focar na leitura e escrita de arquivos de texto, tal como
aqueles editores de texto criam. Em seguida vamos ver como trabalhar
com arquivos de banco de dados, que são arquivos binários, especificamente desenhados para serem lidos
e escritos por softwares de banco de dados.
...
Quan queremos ler ou escrever um arquivo (nos referimos em seu disco rígido), primeiro 
devemos {\br abrir} o arquivo. Ao abrir o arquivo, comunicamos com o sistema operacional,
que sabe onde está os dados armazenados. Quando você abre um arquivo, você está perguntando 
para o sistema operacional onde encontrar o arquivo pelo nome e assegurar-se que ele existe.
Neste exemplo, abriremos o arquivo {\tt mbox.txt}, que deve ser armazenado na mesma pasta
que iniciou o Python. 
...
\section{Arquivos de texto e linhas}
Um arquivo de texto pode ser pensado como uma sequencia de linhas, bem como uma string Python
pode ser pensada como uma sequencia de caracteres. Por exemplo, este é um amostra de um arquivo de texto que registra a atividade de email de vários individuos em uma equipe de desenvolvimento de projeto de código aberto.
...
\section{Buscando através de um arquivo}
...
\item[Arquivo de texto:]
Uma sequencia de caracteres armazenados em no armazenamento permanente como um disco rigido.
...
\begin{ex}
Escreva um programa para ler um arquivo e imprimir o conteúdo
do arquivo (linha a linha) todos em maíusculo. Executando o programa, ficará da seguinte forma:
...
\afterverb
%
você pode baixar o arquivo em 
\url{www.py4inf.com/code/mbox-short.txt}
\end{ex}

\begin{ex}
Escreva um program 
para solicitar um nome de arquivo e em seguida ler o arquivo
e olhar para as linhas do formulário:
...
\begin{ex}
Às vezes quando programadores ficam entediados ou querem ter um pouco de diversão,
eles adicionam um inofensivo {\bf Easter Egg} para o seu programa (\url{en.wikipedia.org/wiki/Easter_egg_(media)}) que solicita ao usuário 
um nome para arquivo para que ele imprima uma mensagem engraçada quando o usuário digitar o 
exato nome do arquivo ``na na boo boo''. O programa deve se comportar normlamente para todos
os outros arquivos que existem e não existem. Aqui está um exemplo de execução do programa.
